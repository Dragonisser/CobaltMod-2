...
public class BlockPortalCaves extends BlockPortal {
	...
	private static Field lastPortalPos;
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockPortalCaves.Size blockportal$size = new BlockPortalCaves.Size(worldIn, pos, EnumFacing.Axis.X);

		if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockPortalCaves.Size blockportal$size1 = new BlockPortalCaves.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	private void setFields() {
		if (lastPortalPos == null || lastPortalVec == null || teleportDirection == null) {
			lastPortalPos = ObfuscationReflectionHelper.findField(Entity.class, "field_181016_an");
			lastPortalVec = ObfuscationReflectionHelper.findField(Entity.class, "field_181017_ao");
			teleportDirection = ObfuscationReflectionHelper.findField(Entity.class, "field_181018_ap");
		}
	}
...
//			teleportDirection = getClassField(Entity.class, "teleportDirection", "field_181018_ap");
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockPortalCaves.Size blockportal$size = new BlockPortalCaves.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if (!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockPortalCaves.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != CMContent.PORTAL_FRAME_CAVES) {
					break;
				}
			}

			Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
			return block == CMContent.PORTAL_FRAME_CAVES ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for (this.height = 0; this.height < 21; ++this.height) {
				for (int i = 0; i < this.width; ++i) {
					...
					if (block == CMContent.PORTAL_CAVES) {
					...
					if (i == 0) {
						...
						if (block != CMContent.PORTAL_FRAME_CAVES) {
							break label24;
						}
			...
			for (int j = 0; j < this.width; ++j) {
				if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != CMContent.PORTAL_FRAME_CAVES) {
					this.height = 0;
					break;
				}
			}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(null) == Material.AIR || blockIn == CMContent.BLUE_FIRE || blockIn == CMContent.PORTAL_CAVES;
		}
		...
		public void placePortalBlocks() {
			for (int i = 0; i < this.width; ++i) {
				...
				for (int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), CMContent.PORTAL_CAVES.getDefaultState().withProperty(BlockPortalCaves.AXIS, this.axis), 2);
				}
			}
		}
	}
